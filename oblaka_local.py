...
def compare_df(new_df): # Смотрим а что же изменилось по сравнению с вчерашними данными?
    old_df = pd.read_excel('Summary '+(datetime.date.today() - datetime.timedelta(1)).strftime("%Y-%m-%d")+'.xlsx', usecols=[0,1,2,3,4,5]) # Берем вчерашние данные
...
if __name__ == '__main__':
    try:
        ...
        writer = pd.ExcelWriter('Summary ' + datetime.date.today().strftime("%Y-%m-%d") + '.xlsx')
